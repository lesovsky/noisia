...
func runChurn(ctx context.Context, conn db.Conn, logger log.Logger, config Config, updateSQL string, lo, hi int, counter *atomic.Int64) error {
	...
	// nothing, so the real WAL written per UPDATE would fall far below the reported
	// payload-written. Random bytes do not compress, so the actual WAL flood matches the
...
func prepare(ctx context.Context, conn db.Conn, tableIdent string, rows, payloadBytes int) error {
	...
	// almost nothing, so the actual WAL written per UPDATE falls far below the reported
	// payload-written. Random bytes do not compress, so the real WAL flood matches the
	// nominal payload size. Filled once and reused for every row — no per-statement cost.
