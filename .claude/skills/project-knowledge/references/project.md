...
- **hot-row-contention** — `--jobs` workers hammer autocommit UPDATEs on a small shared set of "hot" rows so they serialize on the row-lock / `LWLock:BufferContent`, burning CPU on contention while TPS collapses with no single long blocker. The deliberate inverse of wal-flood (shared rows vs disjoint ranges); the climax is observed externally, not in noisia's log.
- **seqscan-storm** — `--jobs` workers repeatedly full-scan a large table whose filter column has no index (`count(*) WHERE payload = 0`), forcing a Seq Scan every query so CPU burns on brute-force read+filter. The brute-force counterpart to hot-row-contention's contention; warms the cache first, self-reports logical bytes scanned, and (with a large `--table-size`) shades from CPU- toward IO-bound. Degradation, not instance death.
