...
- **Seed payload must be incompressible (and inline where on-disk size matters).** A `payload bytea` seeded/churned with a zero-filled `make([]byte, K)` buffer is a trap: at K ≥ ~2 KB the value is TOASTed out-of-line and an all-zeros payload pglz-compresses to almost nothing, so the table's real on-disk size (and the real WAL volume per UPDATE) collapses to a tiny fraction of what `--table-size` / `payload-bytes` imply. Fill the buffer with random (incompressible) bytes once and reuse it; for workloads whose contract maps `--table-size` into the **main heap** (bloat / dirty-page count), keep the row inline by defaulting `payload-bytes` below the TOAST threshold (e.g. 1024). PostgreSQL re-TOASTs the supplied value on every UPDATE (no content dedup), so the buffer need not vary per statement — it only needs to be incompressible. Regression-guard size with `pg_relation_size` (inline) and `pg_total_relation_size` (a ≥2 KB TOAST-mode case that fails on zeros). See ADR-008-2; the bug originated in the copied xminhorizonholder seed template and was fixed across bloatchurn/checkpointstorm/walflood/slotbloat.
...
### Rate attack — symmetric churn fan-out with no singleton driver (the inverse of xmin-horizon-holder)
**File:** `bloatchurn/` (incl. `bloatchurn_test.go`, `bloatchurn_integration_test.go`)
**Shows:** the slow-line `--jobs` per-worker `db.Connect` fan-out reduced to its simplest form — **xmin-horizon-holder MINUS the holder organs** (no `establishHolder`/`runHolder`/`heldSince`/`holderRestarts`, atomics collapse to `churned`/`sessions`). Net-new techniques worth copying: (1) **HOT broken on purpose via an indexed mutated column** — the seed's only index is on `updated_at`, and the churn `UPDATE … SET updated_at = now(), …` forbids HOT on every statement (proven in integration by `n_tup_hot_upd/n_tup_upd < 0.1`) while the monotonic `now()` bloats the btree right edge. (2) **A hot-prefix bound that leaves a live tail** — churn targets `id ∈ [1, max(1, floor(hotFraction·rows))]` (`hotFraction = 0.5` const, not a flag, per ADR-004-1), so the untouched upper half keeps trailing heap pages live and VACUUM cannot truncate the file (the bloat survives for the post-stop repair demo); pass `hotRows` into `runWorkerWithConn` so the bound is unit-testable, and prove the tail is pristine with the non-self-referential invariant `count(DISTINCT updated_at) WHERE id > hotRows == 1`. (3) **`--keep-table`** (the `slot-bloat` `--keep-slot` precedent) preserves the bloated table on graceful exit. (4) **Seed-size fidelity** — incompressible inline payload + the `pg_relation_size` / `pg_total_relation_size` regression guards (see the seed-payload pattern above and ADR-008-2). Honest contract: degradation observed externally (pgstattuple / `n_dead_tup` / size), never instance death; remediable after stop (VACUUM FULL / pg_repack / pgcompacttable / REINDEX CONCURRENTLY).
